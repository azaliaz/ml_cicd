...
def predict(body: PredictRequest):
    ...
    X_df = pd.DataFrame([[getattr(body, n) for n in names]], columns=names)
    try:
        value = float(model.predict(X_df)[0])
    except Exception as e:
        _LOG.exception("predict: ошибка инференса")
        raise HTTPException(status_code=400, detail=str(e)) from e
    _LOG.info("predict: ok median_house_value=%s", value)
    return PredictResponse(median_house_value=value)
